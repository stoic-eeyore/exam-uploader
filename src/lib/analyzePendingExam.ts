...
export async function analyzePendingExam(payload: BasePayload, examId: string) {
  try {
    ...
    console.error(`[analyzePendingExam] Failed for exam ${examId}:`, err)

    // Mark as failed so it doesn't get retried blindly and we can inspect it
    await payload
      .update({
        collection: 'pending-exams',
        id: examId,
      ...
      .catch((updateErr) => {
        ...
        console.error(
          `[analyzePendingExam] Also failed to update status for exam ${examId}:`,
          updateErr,
        )
      })
